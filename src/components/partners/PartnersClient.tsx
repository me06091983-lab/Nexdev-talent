...
function PartnerModal({
  ...
  async function handleSave() {
    if (!form.last_name.trim()) { setError('Numele este obligatoriu.'); return }
    setSaving(true)
    setError('')
    try {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b bg-gray-50 rounded-t-2xl">
          <h2 className="font-semibold text-gray-900 text-sm">{isEdit ? 'Editează partener' : 'Adaugă partener'}</h2>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600 rounded"><X size={18} /></button>
        </div>

        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          {/* Personal */}
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Date personale</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Prenume</label>
              <input value={form.first_name ?? ''} onChange={e => set('first_name', e.target.value)} placeholder="Ion" className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Nume <span className="text-red-400">*</span></label>
              <input value={form.last_name} onChange={e => set('last_name', e.target.value)} placeholder="Popescu" className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          {/* Company */}
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest pt-1">Date firmă</p>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Nume firmă</label>
            <div className="relative">
              <Building2 size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none" />
              <input value={form.name ?? ''} onChange={e => set('name', e.target.value)} placeholder="Firma SRL" className={inputCls + ' pl-8'} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">CUI Firmă</label>
              <input value={form.company_cui ?? ''} onChange={e => set('company_cui', e.target.value)} placeholder="RO12345678" className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Țară firmă</label>
              <div className="relative">
                <Globe size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none" />
                <input value={form.company_country ?? ''} onChange={e => set('company_country', e.target.value)} placeholder="România" className={inputCls + ' pl-8'} />
              </div>
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Cont bancar (IBAN)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Termeni comision <span className="text-gray-300">(opțional)</span></label>
            <textarea value={form.commission_terms ?? ''} onChange={e => set('commission_terms', e.target.value)}
              rows={2} placeholder="Ex: 10% din primul salariu lunar..."
        ...
        <div className="flex gap-2 px-5 pb-5 pt-2">
          <button onClick={onClose} className="flex-1 py-2.5 text-sm border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition-colors">Anulează</button>
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? 'Salvez...' : isEdit ? 'Salvează' : 'Adaugă partener'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-3">
        <p className="text-sm text-gray-500">{partners.length} {partners.length === 1 ? 'partener' : 'parteneri'}</p>
      ...
      {partners.length === 0 ? (
        <div className="glass rounded-2xl p-16 text-center">
          ...
          <p className="text-gray-400">Nu există parteneri înregistrați.</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/40 bg-white/30 text-left">
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Partener</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Contact</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Firmă</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">CUI</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Țară</th>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => setEditing(p)}
                        className="p-1 text-gray-300 hover:text-[#2AA3FF] hover:bg-blue-50 rounded transition-colors" title="Editează">
                        <Pencil size={13} />
                      </button>
                      <button onClick={() => handleDelete(p.id)} disabled={deletingId === p.id}
                        className="p-1 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded transition-colors disabled:opacity-40" title="Șterge">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
